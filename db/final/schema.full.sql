...
-- "Tiered Sufficiency with Signed Evidence".
--
-- ADJUDICATION: the EVIDENCE-CENTRIC proposal (db/evidence-centric/) is the
-- base. The PROPOSITION-CENTRIC proposal (db/0*.sql) contributed its
-- incremental re-grading machinery, its derived refutation test, its
-- append-only enforcement, its publication closure and several point fixes.
-- Defects present in BOTH were repaired here and are marked  [FIX-BOTH].
--
-- The three rules that carry the design:
--   1. Nothing graded lives on a site. Every grade is a row in `proposition`.
--   2. Evidence is SIGNED and BOUND: an observation supports or undercuts
--      exactly one proposition, at a magnitude, against a NAMED alternative.
--   3. The rollup is a cache. Drop core.proposition_rollup and rebuild it
--      exactly with core.recompute_proposition() over the observation rows.
-- =====================================================================
...
-- HTTP surface at all. RLS underneath is defence in depth, not the only
-- defence — requirement 8 is not satisfied by one mechanism.
-- ---------------------------------------------------------------------
...
-- Requirement 7 demands that a claimed place name with NO coordinates be
-- representable without inventing either. It now is, and it is distinct from
-- `non_located` (documented, coordinates genuinely unknown — BES §10.4).
...
-- arithmetic, not a bolted-on cap. Negative cells produce UNDERCUTS rows —
-- this is what makes a published tenant lease list count AGAINST a hardened
-- facility claim rather than merely fail to count for it.
...
-- code, a lookup into one of these tables, a count of rows, a boolean with a
-- written test, or one of exactly two adjudicated judgement calls. These
-- tables hold all the judgement that v0.1 put in a weight vector, relocated
-- from unreproducible per-candidate scoring into auditable versioned lookups.
--
...
-- THE TIER OF A DOCUMENT IS THE TIER OF ITS AUTHOR, NOT ITS HOST, hence
-- host_tier and content_tier are separate columns: The Black Vault and
-- governmentattic are T3 hosts delivering T1 content — record both.
-- ---------------------------------------------------------------------
...
-- DILUTES every member and triggers a CANDIDATE-SET-CHANGE re-grade of the
-- whole set — enforced by trigger further down, not left to a worker.
-- ---------------------------------------------------------------------
...
create table core.entity (
  ...
  -- citation returned for one is a directly measured hallucination. Proposal A
  -- had a canary table but no containment: nothing stopped a canary entity
  -- from being published. This constraint is that containment.
...
-- performing its own citogenesis at the interface layer. So geometry is an
-- ASSERTION with a source, exactly like every other fact — there are no
-- orphan coordinates. Competing assertions coexist; the register picks a
-- render geometry BY RULE, never by overwrite, and keeps the losers visible.
--
-- Proposal A stored one point and one polygon as columns ON the entity: no
-- provenance, no competing assertions, and an overwrite destroyed the prior
-- claim. This table is proposal B's and it is kept.
--
-- [FIX-BOTH] `place_name_only` carries a claimed NAME and no shape at all.
-- Neither proposal could represent "the forum says it is under Mount Ararat"
-- without inventing either a coordinate (B) or a polygon (A).
-- =====================================================================
...
-- the function is not" is two rows here, two grade histories, two badges.
...
-- to proposition_id and to nothing else.
-- =====================================================================
...
-- express that a profile is inapplicable to THIS facility — the MILCON
-- profile is X3 for an appropriated agency and X0 for the Federal Reserve.
-- Without this the Mount Pony and Greenbrier cases score against a
-- denominator that includes record classes which could never have existed.
...
-- confabulating LLM. The stronger guarantee is: a cited document is either
-- RESOLVED TO BYTES or it is arithmetically inert. That guarantee is made
-- enforceable here — receipt_state cannot be VERIFIED unless every element
-- of the chain is present, and membership in V is a GENERATED column derived
-- from it, so no agent can assert its way into the arithmetic.
-- =====================================================================
...
create table core.source_document (
  ...
  -- is not evidence for the claim. It is evidence for ORIGIN and graded there.
  -- This is what sorts Nichols/Montauk and Sauder's 129 DUMBs (V = empty ->
  -- CAP-5 -> F) from Denver International Airport (third-party T1 records of
  -- real facts enter V at D0 -> E).
...
-- the document is about THIS facility. A receipt proves bytes exist; it does
-- not prove the document is about this site, and the commonest real-world
-- failure is a genuine record attributed to the wrong facility.
...
-- at a magnitude that is a lookup, with a sign that can be negative, against
-- a NAMED alternative. Refutation is data, not absence of data: an UNDERCUTS
-- row, a NEUTRAL row and no row at all are three distinguishable states.
--
-- `membership` is GENERATED. Set membership is therefore a schema fact, not
-- an agent's opinion, and the arithmetic is reproducible from the row alone.
-- This is the strongest form of requirement 3 available in a database: the
-- seven exclusions of BES §2.4 are compiled into a stored column.
-- =====================================================================
...
begin
  ...
  -- satisfies the §3.4 gate (a record whose issuer has authority over the
  -- fact and states it on its face is by construction not producible by the
  -- alternative — without this clause documentary sufficiency stalls at C on
  -- condition A3, which is historian fatal #6 reappearing).
...
-- copies, and the register must say so." Saying so is a GRAPH PROPERTY. It
-- is never COUNT(*), and it is never COUNT(DISTINCT lineage_id) over a
-- column an agent wrote either — proposal A's independent_lineages() was
-- literally `count(distinct v.lineage_id)` under a comment claiming it was a
-- graph property. The lineage count below is computed from the edges.
--
-- ---------------------------------------------------------------------
-- [FIX-BOTH]  TERMINATION.  Both proposals wrote their traversals as
--   WITH RECURSIVE ... UNION ALL ... CYCLE node SET is_cycle USING path
--   with a depth cap of 24 (B) or 25 (A).
--
-- The CYCLE clause is necessary and NOT sufficient. It prunes a branch when a
-- node repeats ON THAT BRANCH; it does nothing about the number of distinct
-- simple paths, which in a dense citation cluster grows exponentially in
-- depth. Measured on this exact schema shape, on a 40-document / 380-edge
-- citogenesis cluster (a small one — the domain routinely produces larger):
--
...
--
-- Both proposals run this at depth 24-25 inside the ingest path. That is a
-- production outage on the first real citogenesis cluster the register
-- ingests, and citation cycles are the norm in this domain, not the
-- exception. Neither CYCLE clause nor depth cap prevents it.
--
-- The fix is to deduplicate on the NODE rather than enumerate PATHS:
--   * reachability recursions use UNION (not UNION ALL) keyed on the node,
--     so the working set is bounded by |V| and terminates in <= |V| steps;
--   * the depth-carrying recursion dedups on (node, depth), bounded by
--     |V| x max_depth;
--   * no path array is carried in the recursive term — path arrays are what
--     make every path distinct and therefore defeat deduplication entirely;
--   * a node budget is a third, independent stop.
...
-- observation (Wayback CDX collapse=digest) and SAYS SO. A closed citation
-- loop with no dated entry point is itself a finding about the claim, and the
-- register does not invent a terminus it does not have.
...
-- "independent lineage" is actually computed (requirement 4).
-- UNION deduplication is the termination guarantee: on a finite graph the
-- working set empties in at most |V| iterations no matter how many cycles
-- exist. Citogenesis loops ARE cycles; this is the code path that survives.
-- ---------------------------------------------------------------------
...
-- T5 testimony, subsequently cited as though primary. The whole loop is ONE
-- lineage, the laundered proposition is capped at E, and the flag attaches to
-- the PROPOSITION — which is exactly where the Mount Weather "underground
-- city" claim had nowhere to live under v0.1.
-- ---------------------------------------------------------------------
...
-- ASSERTING, never whoever is quoted. An anonymous claim ABOUT what a named
-- person said is one lineage with the claimant, not with the person. This
-- closes the cheapest lineage-seeding attack there is: attributing invented
-- testimony to two real, dead, findable people.
-- ---------------------------------------------------------------------
...
-- affirmatively resolves against it. A mundane explanation that merely
-- ACCOUNTS FOR the signals removes their probative value; it does not
-- disconfirm the claim (BES §8).
-- =====================================================================
...
-- LLM writing state='R3' produced an R grade. The state stored here is a
-- PROPOSAL; core.derive_refutation() below re-tests the conditions against
-- the observation rows and only its verdict reaches a grade.
...
-- `coalesce(v_sci,0) < 0.5`, forcing X. A proposition for which NO record
-- class is expected to exist — every applicable ERP profile at X0, which is
-- the NORMAL condition for a classified facility and is exactly the Mount
-- Pony and Greenbrier-1991 cases — would therefore be pinned at X forever,
-- unable ever to publish the very silence reading the model exists to state.
-- An empty denominator means "there was nothing to search", which is
-- complete, not incomplete: SCI = 1.0.
-- ---------------------------------------------------------------------
...
-- Grafted from proposal A, which tested the conditions; proposal B trusted
-- REFUTER's label. Every branch requires an AFFIRMATIVE row: expected-record
-- UNDERCUTS rows can never alone license R, however deep the negative stack.
-- Without that gate the register quietly refutes real facilities whose cover
-- story is good — its characteristic failure mode — and returns R on the
-- Sauder 129-DUMB corpus purely from two X3 negatives where F is correct.
-- ---------------------------------------------------------------------
...
-- bands top-down, then the caps, then the clamp, then the SCI floor.
-- Every step is deterministic given the observation rows. The function lives
-- in the database because the grade must be recomputable from the rows alone,
-- by anyone, forever; an external scorer that cannot be re-run is an
-- unauditable scorer.
-- =====================================================================
...
begin
  ...
  ---------------------------------------------------------------------
  -- A1: >=1 V row at D4 with CLAIM-PROPERTY (the §3.4 gate), OR two D3
  -- claim-property rows in two INDEPENDENT lineages.
  ...
  if core.grade_rank(awarded) is not null then
    ...
    -- constraint in the anti-gaming ledger: no FUNCTION claim can exceed E
    -- without a verified, instance-scope, subject-bound, claim-property row.
    if n_vclaim = 0 then
  ...
  -- X is not a low grade; it is the absence of one, and must render visually
  -- distinct from F. An R is never withheld — a refutation that has been
  -- established does not become unestablished for want of further searching.
  ---------------------------------------------------------------------
...
-- event carrying the full condition vector, the pinned table versions, and
-- the exact observation set in scope when it ran. core.proposition_rollup is
-- a materialised read model: drop it and core.recompute_proposition()
-- rebuilds it exactly. The grade is never STORED knowledge, only CACHED
-- knowledge.
-- =====================================================================
...
create table core.proposition_rollup (
  ...
  -- moment evidence lands that does not change the letter. Publication must
  -- test "was this grade computed from the evidence on file", which is a
  -- property of the CACHE, so the hash lives here too and is refreshed on
  -- every recompute whether or not the grade moved.
...
begin
  ...
  -- DUCC (PROGRAM A / EXIST R) and Dulce (ORIGIN A / FUNCTION R)
  -- representable at all.
  --
  -- Proposal A computed this against the parent's rollup with no ordering
  -- guarantee, so regrading FUNCTION before EXIST silently used a stale
  -- parent and nothing ever re-fired. Proposal B wrote a clamp_event table
  -- that no code path ever inserted into, and applied the clamp only in a
  -- read-time helper the map view did not call. Here the clamp is applied at
  -- recompute, recorded as an event, and the enqueue trigger below re-queues
  -- every child whenever a parent's grade moves.
...
-- own document date PRECEDES the register's publication of the candidate.
-- Downward movement carries no such restriction. The failure mode is
-- inflation, so the ratchet runs one way against it.
--
-- [FIX-B] Proposal B's ratchet blocked the rise if ANY counted observation
-- postdated publication. That is the wrong polarity: one unrelated recent
-- blog post in V would freeze a proposition against a genuine new archival
-- find forever. The test belongs on the rows that actually MOVED the grade,
-- which is proposal A's reading and is adopted here.
...
begin
  ...
  -- re-queueing on it would leave the queue permanently non-empty after every
  -- publish — which in turn makes "is anything stale?" unanswerable. Skip the
  -- enqueue when publication_state is the ONLY thing that moved.
  if tg_op = 'UPDATE'
...
-- EXIST proposition ENQUEUES its children, because their monotone clamp has
-- just been invalidated. A plpgsql FOR loop runs against the snapshot its
-- query took, so rows inserted during the loop are invisible to it and the
-- children would silently keep a stale clamped grade until some later,
-- unrelated write happened to re-queue them. Passes repeat until the queue
-- stops producing work; p_max_passes is the termination guarantee against a
-- pathological enqueue cycle.
...
--
-- "Every claim carries a citation" is satisfied 100% of the time by a
-- confabulating LLM. The stronger guarantee the register needs is that
-- nothing reaches the public projection whose grade was not computed from
-- the evidence actually on file, and that no counted row is unresolved.
-- core.assert_publishable() is that guarantee, and ops.publish_entity() is
...
begin
  ...
  -- only fail if the generated column and the receipt disagree — but it is
  -- asserted here anyway, because the whole point is that the guarantee is
...
begin
  ...
  -- the laundered claim while hiding the T5 forum post it rests on — exactly
  -- the failure it exists to expose. The walk deduplicates on the node, so a
  -- cyclic closure terminates (see §9).
...
-- stored geometry AND the grade of the LOCATE proposition. Below band C a
-- point is structurally unavailable: the renderer cannot draw one because
-- the API never emits one. A precise pin manufactured from imprecise
-- evidence is the register performing its own citogenesis at the interface.
-- =====================================================================
...
-- walk the evidence graph. Materialised views DO NOT ENFORCE RLS, so the
-- WHERE clause below IS the security boundary for this object and is written
-- to be read as such.
-- ---------------------------------------------------------------------
...
--  (a) it snapped geom_3857, which for a region or uncertainty-circle feature
--      is a POLYGON. Snapping a polygon to a grid yields a snapped polygon,
--      and two nearby polygons snap to different polygons — so region
--      features never clustered at all. Clustering keys on label_point_3857.
--  (b) its cluster_id was row_number(), which renumbers every cluster on
--      every refresh, so no client can cache a cluster across a refresh. The
--      key is now the grid cell itself, which is stable under ingest.
-- ---------------------------------------------------------------------
...
--     adjudication task would leak an unpublished candidate.
-- =====================================================================
...
--   1. PostgREST is pointed at `api` only; core/ingest/registry have no HTTP
--      surface at all.
--   2. RLS on every table, with anon policies requiring PUBLISHED all the
--      way up the chain.
...
-- proposal are closed here. Each was `using (true)`:
--
--   core.witness           — real people's names, the resolvability
--                            adjudication, the record kind used to resolve
--                            them, and free-text adjudicator notes, readable
--                            for candidates that were never published. This
--                            is both an adjudication-state leak and a
--                            private-individual exposure.
--   core.lineage           — every lineage, its terminus document and its
--                            agent_model_family, for unpublished candidates.
--   core.document_citation — the ENTIRE citation graph regardless of
--                            publication. Because the edges carry document
--                            UUIDs, an anonymous reader could enumerate the
--                            shape and size of the evidence base behind every
--                            unpublished candidate in the register.
--
-- All three are now gated on the published closure. The citation graph and
-- the lineage analysis remain fully public FOR PUBLISHED ENTRIES — they are
-- the product — but they no longer describe work in progress.
-- ---------------------------------------------------------------------
...
--     ERP table load from the W0 JSON on top of this.
-- =====================================================================
