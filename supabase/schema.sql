-- =====================================================================
-- BUNKERS REGISTER — THE ADJUDICATED SCHEMA
-- =====================================================================
-- PostgreSQL 15+ / PostGIS 3.x (Supabase). Implements BES v0.2 as
-- specified in docs/GRADING.md. Prose companion: docs/SCHEMA.md.
--
-- APPLY WITH:
--     psql -v ON_ERROR_STOP=1 -f supabase/schema.sql
--     psql -v ON_ERROR_STOP=1 -f supabase/test_acceptance.sql
--
-- VERIFIED: applies clean from an empty database on PostgreSQL 16.13 +
-- PostGIS 3.4 (EXIT=0); the 65-assertion acceptance suite passes 65/65
-- with 0 errors. Objects created: 56 tables, 9 api views, 2 materialised
-- views, 51 functions, 167 indexes, 101 RLS policies, 88 CHECK
-- constraints, 22 triggers.
--
-- ---------------------------------------------------------------------
-- THE ONE THING TO UNDERSTAND BEFORE READING FURTHER
-- ---------------------------------------------------------------------
-- This register is populated ENTIRELY by LLM agents. They confabulate,
-- hallucinate plausible document identifiers, and misattribute real
-- documents to the wrong facility. Every design decision below assumes
-- agent output is untrusted by default.
--
-- Consequently, four things that look like they could be columns are not:
--
--   * `core.observation.membership` (V/U/INERT/V0) is a STORED GENERATED
--     column compiling the seven BES §2.4 exclusions. No agent can assert
--     set membership; it is a schema fact.
--   * There is NO lineage_id column on core.observation. Independence is
--     a graph property computed over citation edges by
--     core.lineage_components(). A count over an agent-writable column is
--     not independence, however the comment above it is worded.
--   * `core.refutation.proposed_state` is what a REFUTER agent wrote.
--     The grade uses core.derive_refutation(), which re-tests R1/R2/R3
--     against the rows. An LLM writing 'R3' produces nothing.
--   * There is no site-level score column anywhere. The composite does
--     not exist. api.proposition_badge returns N badges.
--
-- ---------------------------------------------------------------------
-- STRUCTURE (section -> what it guarantees)
-- ---------------------------------------------------------------------
--  1-2  Extensions, four schemas (registry/core/ingest/api), roles,
--       closed vocabularies, immutable helpers. core.grade_rank() leaves
--       R and X UNRANKED: they are not low grades, they are different
--       epistemic objects, so they are neither capped nor clamped.
--       core.ea_matrix() is the BES §4.4 fallback with a hard D3 ceiling
--       — D4 is unreachable by inference and requires the §3.4 gate.
--       The firewall is arithmetic.
--
--  3    registry.*  — the curated, versioned, reviewed-write tables
--       (tier ladder / diagnosticity catalog / ERP profiles / candidate
--       sets) plus identifier grammars, base rates, scorer models.
--       Country and admin geography are ROWS, so non-US expansion is
--       additive. registry.corpus separates host_tier from content_tier
--       (The Black Vault is a T3 host delivering T1 content) and carries
--       egress state for BES §2.6 degraded-verification mode.
--
--  4-5  core.entity (a container, nothing graded) + is_canary
--       containment; core.entity_alias (the subject-binding match table
--       — entity resolution is a VERIFICATION INPUT); DISTINCT-FROM
--       relations; core.geometry_assertion — competing, sourced,
--       versioned geometry, including the place_name_only precision for
--       a claimed place name with no coordinates at all.
--
--  6    core.proposition — the atomic graded unit. null_code NOT NULL,
--       predicate_args validated per class against closed vocabularies,
--       PROGRAM/ORIGIN clamp-exempt by trigger, one EXIST per entity so
--       the clamp parent is unambiguous. core.proposition_erp makes ERP
--       applicability per-proposition, not per-class.
--
--  7    core.source_document (three orthogonal provenance dimensions),
--       core.retrieval_receipt (RESOLVE-OR-DIE as a CHECK constraint),
--       core.quoted_span (probative + subject-binding), ingest.lead
--       (untrusted identifiers never become citations), search receipts
--       (absence is not citable without a receipt for the absence).
--
--  8    core.observation — THE ATOM. Generated `membership` compiles the
--       seven BES §2.4 exclusions; generated `exclusion_reason` renders
--       why a row is inert. D4 is the six-condition §3.4 gate and nothing
--       else. core.forbid_delete() on ten tables: nothing is deleted.
--
--  9    The citation graph, lineage components, cycle-safe AND
--       path-explosion-safe origin tracing, citogenesis detection,
--       attestation custody.
--
-- 10    Derived null_state, search completeness, silence reading, and
--       core.derive_refutation() — R re-tested from the rows.
--
-- 11    core.evaluate_proposition() — GRADE(P) per BES §9.4.
--
-- 12-13 Append-only grade events, the rollup cache, the clamp, the
--       one-way ratchet, the convergent regrade queue, reconstruction.
--
-- 14    core.assert_publishable() + ops_publish_entity(): the only path
--       to public visibility, with the transitive citation closure.
--
-- 15    core.render_geometry() (a point requires LOCATE at band C),
--       api.map_feature / api.map_cluster, viewport RPC, vector tiles.
--
-- 16-18 Ingest telemetry, RLS (101 policies), the published projection.
--
-- 19    Seed: rubric + table versions, countries, the twelve enumerated
--       nulls (BES §4.5), base-rate readings (BES §6.5), the
--       universal-D0 catalog across every typology, a slice of the
--       buried-rural catalog, and seventeen expected-record profiles
--       seeded from the W0 `gaps` sections (the NPRC fire, the FCC/NTIA
--       split, FRPP's national-security exclusion, NEPA categorical
--       exclusions, the federal-construction permit exemption,
--       federal-to-federal transfers, commercial cover).
--
-- ---------------------------------------------------------------------
-- TERMINATION, VERIFIED RATHER THAN ASSUMED
-- ---------------------------------------------------------------------
-- Both input schema proposals wrote origin tracing as
--   WITH RECURSIVE ... UNION ALL ... CYCLE node SET is_cycle USING path
-- at depth 24-25, and both commented that the CYCLE clause made it safe.
-- It does not. The CYCLE clause prunes a branch when a node repeats ON
-- THAT BRANCH; it says nothing about the NUMBER OF DISTINCT SIMPLE PATHS,
-- which grows exponentially with depth in a dense graph, and UNION ALL
-- plus a path array makes every path a distinct row so deduplication
-- cannot help. Measured on a 40-document / 380-edge citogenesis cluster
-- (a small one for this domain):
--
...
--
-- Citation cycles are the NORM here, not the exception: a citogenesis
-- loop literally IS a cycle, and detecting them is the register's
-- purpose. The first real echo chamber ingested takes the worker down.
--
-- core.trace_origin below deduplicates on the NODE rather than
-- enumerating PATHS: UNION (not UNION ALL) keyed on (node, depth), no
-- path array in the recursive term (path arrays are precisely what defeat
-- deduplication), and a node budget as a third independent stop. Path
-- reconstruction moved out of the recursion into core.origin_path, a
-- bounded loop of single-row lookups with a visited set. Same fixture,
-- same depth 25: 20 nodes in 9.6 ms. core.lineage_components resolves the
-- 40-node cyclic cluster in 2.6 ms.
-- =====================================================================
